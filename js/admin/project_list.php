...
<body>
                                ...
                                <table id="example" class="table header-border table-responsive-sm table-bordered"
                                    ...
                                    <tbody>
                                        ...
                                        while ($row = mysqli_fetch_array($res)) {
                                            ...
                                            // Set the checkbox status based on the current value in the database (1 for active, 0 for inactive)
                                            $status_checked = ($row['status'] == 1) ? 'checked' : ''; // Active if 1 feature_status
    ...
    // Event listener for the king status toggle checkbox
    $('.king-status-toggle').change(function() {
        var projectId = $(this).data('project-id'); // Get the project ID
        var newStatus = $(this).prop('checked') ? 1 : 0; // 1 if checked (active), 0 if unchecked (inactive)
        
        // Make an AJAX request to update the king_status in the database
        $.ajax({
            url: 'update_project_king_status.php', // PHP file that handles the status update
            type: 'POST',
            data: {
            ...
            success: function(response) {
                if(response == 'success') {
                    // Optionally: You can display a message or perform other actions upon successful update
