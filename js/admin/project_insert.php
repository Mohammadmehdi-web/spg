...
if (isset($_POST['submit'])) {
    ...
    try {
        ...
        // Handle multiple file uploads for photo_gallery (same product_id is used)
        if (isset($_FILES['photo_gallery'])) {
            foreach ($_FILES['photo_gallery']['name'] as $key => $photo_name) {
                // Generate unique name for each photo
                $unique_photo_name = uniqid() . "_" . basename($photo_name);

                if ($_FILES['photo_gallery']['error'][$key] == 0) {
                    $target_file = $target_dir . $unique_photo_name;

                    if (move_uploaded_file($_FILES['photo_gallery']['tmp_name'][$key], $target_file)) {
                        // Insert photo into project_images table
                        $sql = "INSERT INTO project_images (project_id, photo_gallery) VALUES (?, ?)";
                        $stmt = $con->prepare($sql);
                        $stmt->bind_param("is", $project_id, $unique_photo_name);

                        if (!$stmt->execute()) {
                            throw new Exception("Error inserting photo: " . $stmt->error);
                        }
                    } else {
                        throw new Exception("Error uploading photo: $photo_name");
                    }
                } else {
                    throw new Exception("Error uploading photo: " . $_FILES['photo_gallery']['error'][$key]);
                }
            }
        }
