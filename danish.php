...
<body class="preload-wrapper">
    ...
    <?php endforeach; ?>
                        ...
                        <!-- Pagination Controls -->
                       <ul id="pagination" class="wg-pagination tf-pagination-list"></ul>
            ...
            success: function(response) {
                ...
                if (response.projects.length > 0) {
                    $.each(response.projects, function(index, row) {
                        ...
                        <div class="card-product grid">
                            <div class="card-product-wrapper">
                                ${row.king_status == 1 ? `<div class="king-crown-icon" style="position: absolute; right: 5px; z-index: 10;">
                                    <img width="40" height="40" src="https://img.icons8.com/color/48/fairytale.png" alt="King Crown">
                                </div>` : ''}

                                <a href="project/${row.pro_url}" class="product-img">
                                    ${row.feature_status == 1 ? `<div class="king-crown-icon" style="position: absolute; top: 10px; right: 10px; z-index: 10;">
                                        <span class="badge rounded-pill bg-danger">Feature</span>
                                    </div>` : ''}
    ...
    function generatePagination(totalPages) {
        ...
        if (totalPages > 1) {
            for (let i = 1; i <= totalPages; i++) {
                paginationHtml += `<li class="pagination-item ${i === currentPage ? 'active' : ''}">
...
$sql = "SELECT add_project.year, project_images.image, add_project.pro_tile, add_project.highlight_text, 
...
// Apply only the first non-empty filter
if (!empty($industry)) {
...
// Response with data & pagination info
$response = [
    'projects' => $projects,
